...
import com.roche.ota.model.manager.BluetoothUtils.bytesToHexString
import com.roche.ota.utils.showToast
import java.util.*
import kotlin.collections.ArrayList


class BleConnectionHelper private constructor(context: Context) {

    val TAG = "BleConnectionHelper"
    private var mContext: Context = context

    companion object : SingletonHolder<BleConnectionHelper, Context>(::BleConnectionHelper)
     ...
     * 初始化蓝牙
     */
    private fun initBluetooth(mContext: Context) {
        //初始化ble设配器
     ...
     * 连接
     */
    fun connection(mac: String) {
        macAddress = mac
        Log.e(TAG, "蓝牙mac:$macAddress")
    ...
    fun readCharacteristic(
        ...
        characteristicValueView: TextView
    ) {
        if (isConnected) {
            mHandler.post {
...
//        if (characteristic.uuid == UrlConstant.uuid_ota2) {
//            Log.e(TAG, "WRITE_TYPE_NO_RESPONSE  已经设置---------------")
...
//                mListener?.onNoResponseWriteType(success)
//            }
//        }
...
//        }
    ...
    private fun connectionStateChange(status: Int, newState: Int) {
        Log.e(TAG, "Client  status = $status  newState = $newState")
        ...
        when (status) {
            BluetoothGatt.GATT_SUCCESS -> {
                ...
                mMainHandler.post {
                    //发现服务回调
                    if (mListener != null) {
                        mListener?.discoveredServices(mGattServiceList)
                    }
                }
            }
            else -> {
...
//                    Log.e(TAG, "写入成功的回调:WRITE_TYPE_NO_RESPONSE  ${characteristic.uuid}")
//                    mMainHandler.post {
                    //读取特征值回调
                    if (mListener != null) {
                        mListener?.onNoResponseWriteType(
                            true, characteristic.uuid.toString()
                        )
                    }
//                    }
                    return
                ...
                characteristic?.let { characteristic ->
                    ...
                    mMainHandler.post {
                        //读取特征值回调
                        if (mListener != null) {
                            Log.e(TAG, "写入成功的回调:$stringBuilder")
                            mListener?.writeCharacteristic(
                                bytesToHexString(characteristic.value),
                ...
                if (characteristic?.writeType == WRITE_TYPE_NO_RESPONSE) {
                    Log.e(TAG, "写入失败的回调:WRITE_TYPE_NO_RESPONSE")
                    mMainHandler.post {
                        //读取特征值回调
                        if (mListener != null) {
                            mListener?.onNoResponseWriteType(
                                false, characteristic.uuid.toString()
                            )
                        }
                    }
                    return
    ...
    private fun characteristicChanged(characteristic: BluetoothGattCharacteristic?) {
        characteristic?.let { characteristic ->
            ...
            mMainHandler.post {
                //读取特征值回调
                if (mListener != null) {
                    Log.e(TAG, "通知回调:$stringBuilder")
                    mListener?.characteristicChange(
                        bytesToHexString(characteristic.value),
                        characteristic.uuid.toString()
                    )
                }
...
//                mDescriptorValueView = null
    ...
    private inner class daqiBluetoothGattCallback : BluetoothGattCallback() {
        ...
        override fun onCharacteristicWrite(
            ...
            //抛给工作线程处理 -------------
//            mHandler.post {
//            Log.e(TAG, "onCharacteristicWrite 当前线程:      ${Thread.currentThread().name}")
...
//            }
    ...
    interface BleConnectionListener {
        ...
        //发现服务
        fun discoveredServices(data: ArrayList<BluetoothGattService>)

        //读取特征值
        fun readCharacteristic(data: String)

        //写入特征值
        fun writeCharacteristic(data: String, uuid: String)
        ...
        //写入特征值
        fun characteristicChange(data: String, uuid: String)

        //设置了 属性的回调
        fun onNoResponseWriteType(data: Boolean, uuid: String)
    }
